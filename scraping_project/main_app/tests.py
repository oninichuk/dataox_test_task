...
class ArticleTestCase(TestCase):
    ...
    def test_collect_ft_articles_primary_data(self):
        ...
        self.assertTrue(
            primary_articles is not False,
            "Some exception was thrown, please check logs",
        )

        if primary_articles:
            print(f"Found {len(primary_articles)} articles")
            self.assertTrue(
                isinstance(primary_articles[0]["url"], str) and primary_articles[0]["url"].find("http") == 0,
                "Didn't retrieve article URL",
            )
            self.assertTrue(
                isinstance(primary_articles[0]["html"], str) and primary_articles[0]["html"].find("<body") > -1,
                "Didn't retrieve article HTML",
            )
    ...
    def test_async_requests_manager(self):
        ...
        if primary_articles:
            ...
            if requests_manager.results:
                self.assertTrue(
                    isinstance(requests_manager.results[0]["url"], str) and requests_manager.results[0]["url"].find("http") == 0,
                    "Didn't retrieve article URL",
                )
                self.assertTrue(
                    isinstance(requests_manager.results[0]["html"], str) and requests_manager.results[0]["html"].find("<body") > -1,
                    "Didn't retrieve article HTML",
                )
    ...
    def test_parser_manager(self):
        try:
            with open("main_app/test_articles/article1.html", "r") as file1:
                article1 = file1.read()
                parser_manager = FTParserManager(
                    [
                        {
                            "url": "https://ft.com/content/specific_article",
                            "html": article1,
                        }
                    ]
                )
